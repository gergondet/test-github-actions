...
struct DataStore
{
  ...
  void remove(const std::string & name)
  {
    ...
    if(it == datas_.end())
    {
      std::cerr << "No element named " << name << "\n";
      return;
    }
    datas_.erase(it);
  }
...
private:
  ...
  const T & safe_cast(const Data & data, const std::string & name) const
  {
    ...
    {
      std::cerr << "Object for key " << name << " does not have the same type as the stored type. Stored " << data.type() << " but requested " << type_name<T>() << "\n";
      throw std::runtime_error("BYE");
    }
  ...
  RetT safe_call(const std::string & name, ArgsT &&... args) const
  {
    ...
    {
      std::cerr << "Function for key " << name << " does not have the same type as the stored type. Stored " << data.type() << " but requested " << type_name<fn_t>() << "\n";
      throw std::runtime_error("BYE");
    }
...
int main()
{
  mc_rtc::DataStore store;
  store.make_call("test", [](const mc_rbdyn::Robot &) { std::cout << "Hello world!\n"; return sva::PTransform<double>{}; });
  mc_rbdyn::Robot r{};
  const auto & robot = r;
  store.call<sva::PTransform<double>>("test", robot);
  return 0;
}
